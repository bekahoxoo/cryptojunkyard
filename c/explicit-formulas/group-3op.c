...
bool field_is_zero(const field k) {
  return (memcmp(k, field_zero, field_BYTES) == 0);
}
...
bool scalar_is_zero(const scalar k) {
  return (memcmp(k, scalar_zero, scalar_BYTES) == 0);
}
...
bool is_zero(const group *p) { return memcmp(p->Z, field_zero, field_BYTES); }
...
bool is_on_curve(const group *p) {
  ...
  return (memcmp(lhs, rhs, field_BYTES) == 0);
}
...
void group_add(group *r, const group *p, const group *q) {
  ...
  if (memcmp(p->X, q->X, field_BYTES) == 0) {
    if (memcmp(p->Y, q->Y, field_BYTES) == 0) {
      group_double(r, p);
      return;
    } else {
      *r = group_zero;
  ...
  if (memcmp(q->Z, field_one, field_BYTES) == 0) {
    group_madd(r, p, q);
    return;
  }
  if (memcmp(p->Z, field_one, field_BYTES) == 0) {
    group_madd(r, q, p);
    return;
  }
